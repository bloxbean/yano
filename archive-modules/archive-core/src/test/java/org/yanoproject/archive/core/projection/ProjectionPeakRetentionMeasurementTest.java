...
import org.yanoproject.archive.core.dataset.TransactionFact;
import org.yanoproject.archive.core.dataset.UtxoHistoryFact;
...
class ProjectionPeakRetentionMeasurementTest {
    ...
    private static ProjectionSection accountEventSection(long block) {
        List<org.yanoproject.archive.core.dataset.AccountEventFact> events = new ArrayList<>();
        for (int i = 0; i < 20; i++) {
            ...
            byte[] hash = new byte[32];
            hash[0] = (byte) i;
            events.add(new org.yanoproject.archive.core.dataset.AccountEventFact(
    ...
    private static ProjectionSection addressSection(long block) {
        List<org.yanoproject.archive.core.dataset.AddressParticipationFact.Transaction> txs =
                new ArrayList<>();
        long participations = 0;
        for (int t = 0; t < 20; t++) {
            List<org.yanoproject.archive.core.dataset.AddressParticipationFact.Participation> parts =
                    new ArrayList<>();
            for (int i = 0; i < DENSE_ITEMS / 4; i++) {
                byte[] key = new byte[28];
                key[0] = (byte) i;
                key[1] = (byte) block;
                parts.add(new org.yanoproject.archive.core.dataset
                        .AddressParticipationFact.Participation(
                        i % 2 == 0 ? "INPUT" : "OUTPUT",
                        new org.yanoproject.archive.core.dataset.AddressSubjectRows.Participant(
                                key, "addr_test1" + block + "_" + i, key, "key", key)));
            }
            byte[] txHash = new byte[32];
            txHash[0] = (byte) t;
            txHash[1] = (byte) block;
            txs.add(new org.yanoproject.archive.core.dataset
        ...
        byte[] encoded = ProjectionFactCodec.encodeAddressParticipations(
                new org.yanoproject.archive.core.dataset.AddressParticipationFact(txs));
