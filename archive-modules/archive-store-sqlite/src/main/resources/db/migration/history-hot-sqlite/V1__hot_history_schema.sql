...
CREATE INDEX resolver_outputs_created
  ON resolver_outputs(created_block_number)
...
CREATE TABLE resolver_spends (
    ...
    PRIMARY KEY (referenced_tx_hash, referenced_output_index),
    FOREIGN KEY (referenced_tx_hash, referenced_output_index)
      REFERENCES resolver_outputs(tx_hash, output_index) ON DELETE CASCADE
);
CREATE INDEX resolver_spends_block ON resolver_spends(spending_block_number);
...
CREATE TABLE pointer_registrations (
    ...
    PRIMARY KEY (dataset, pointer_slot, pointer_tx_index, pointer_cert_index)
);
CREATE INDEX pointer_registrations_credential
  ON pointer_registrations(dataset, credential_type, credential);
...
CREATE TABLE pointer_deregistrations (
    ...
    PRIMARY KEY (dataset, credential_type, credential,
                 block_number, slot, tx_index, cert_index)
);
CREATE INDEX pointer_deregistrations_credential
  ON pointer_deregistrations(dataset, credential_type, credential,
                             block_number, slot, tx_index, cert_index);
