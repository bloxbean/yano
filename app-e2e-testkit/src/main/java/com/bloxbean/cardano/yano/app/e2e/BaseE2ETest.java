...
public abstract class BaseE2ETest {
    ...
    protected void fundAddress(String address, long adaAmount) throws Exception {
        ...
        String body = objectMapper.writeValueAsString(new FundPayload(address, adaAmount));
    ...
    protected void waitForTransaction(Result<String> result) throws Exception {
        ...
        for (int i = 0; i < 60; i++) {
            try {
                ...
                // Retry while the transaction propagates.
            }
            Thread.sleep(500);
        }
    ...
    protected void checkIfUtxoAvailable(String txHash, String address) throws Exception {
        ...
        for (int i = 0; i < 30; i++) {
            try {
                ...
                boolean found = utxos.stream().anyMatch(utxo -> utxo.getTxHash().equals(txHash));
                if (found) {
                    log.info("UTXO found for tx {} at {}", txHash, address);
                    return;
                }
            } catch (Exception ignored) {
                // Retry while the UTXO propagates.
            }
            Thread.sleep(1000);
        }
