...
`appchain-evidence-registry` is the ADR-013 reference workflow for a product-batch
inspection or compliance certificate. It deliberately lives under `examples`:
it demonstrates a secure composition of first-party connectors without claiming
to be a complete DPP implementation or a generic workflow engine.
