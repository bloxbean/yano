...
| `yano.app-chain.indexer.storage.path` | appchain-indexers | Process-relative rebuildable app-chain index root |
...
The app has three test tiers: unit, integration, and end-to-end (e2e).
...
Runs tests tagged with `@Tag("integration")`. These require a running devnet instance (`cd app && ./start-devnet.sh`).

### E2E Tests

End-to-end tests exercise the full app (transaction submission, script evaluation, REST APIs, devnet features). The tests auto-start a devnet instance via `@QuarkusTest` — no manual setup needed.
