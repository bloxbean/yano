...
4. **Triage failures.** Read `qa/results/<run-id>/results.json`. For every test whose
   status is `FAIL` or `TIMEOUT`, investigate it (use sub-agents in parallel if
   available), read-only, with this brief:
...
6. **Share.** `qa/results/<run-id>/report.html` is the shareable page (self-contained
   apart from Google Fonts); `report.md` suits a PR comment. Tell the user both paths.
7. **Reply** with the bottom line (all passed, or how many need attention), a short
   table of the non-passing tests with their one-line cause, the report paths, the
