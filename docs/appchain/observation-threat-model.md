...
| Wake/report ingress abuse | Unscoped full API key required even with broad auth disabled; raw subscription-ID hints are coalesced. External reports have a separate eight-check/16-per-second admission budget. Acceptance is not proof of validity, durability, freshness or finality. |
| Re-diffusion amplification | Profile-derived count/byte budgets, bounded pending/recent caches, ten-second retry cooldown and unexpired envelope reuse. Queue saturation can delay delivery; journals remain authoritative for retries. |
| Tick-pool flooding | Dedup by member/anchor, one latest retained hint per member, 60-second retention, profile-derived pool cap and at most one quarter of ordinary pool capacity. |
| Cross-topic sequence starvation | Generic results and ticks have independent member/topic replay floors; they cannot retire pending ordinary effect results or governance votes. |
| SSRF/resource exhaustion | Restricted HTTPS destination/TLS/redirect/framing limits, a shared DNS-to-body deadline, bounded proof parsers, workers, journal capacity and indexed scheduling. DNS isolation has four workers/64 queued lookups; non-interruptible platform resolution can exhaust that bounded shared service but cannot retain acquisition workers indefinitely. Operator-installed custom plugins remain trusted code. |
