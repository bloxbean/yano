# ADR-040: Swagger UI API Groups (Core / App Chain / Devnet / Admin)
...
- Observed on the built `yano.jar` (2026-08-24, after merging the ADR-034
  archive/wallet work from main): 54 paths in `core` (including the new
  `/addresses`, `/events`, `/history` wallet surface), 54 in `app-chain`,
  10 in `devnet`, 25 in `admin`, 140 in the default document; the Swagger
  UI index carries the five `urls` entries with
