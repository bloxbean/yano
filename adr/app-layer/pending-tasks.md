...
| **Block proposal can exceed `maxMessageBytes` → follower rejects → consensus stalls** (found by the cluster load test, 2026-07-11): the proposer's block-bytes cap is `maxMessageBytes × 64` (~4 MiB) but a proposal is diffused as ONE app message, and the inbound transport rejects any body > `maxMessageBytes` (default 64 KiB). So a full `block.max-messages`-sized block (default 500) whose serialized proposal exceeds 64 KiB is silently dropped by followers; the round times out forever at 1-of-N votes. Workaround: raise per-chain `max-message-bytes` (the cluster demo sets 1 MiB) or lower `block.max-messages`. Fix options: cap the block's byte size to fit `maxMessageBytes` in `selectMessages`, and/or exempt consensus `~consensus/propose` messages from the per-message limit (using the transport frame limit instead), plus fail-fast config validation | cluster load-test finding; engine `AppChainSubsystem` block-bytes cap (`maxMessageBytes*64`) vs inbound `maxMessageBytes` check | Before recommending large `block.max-messages`, or the next throughput/robustness pass |
